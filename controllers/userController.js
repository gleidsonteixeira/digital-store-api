...
async function logar(data){
    try {
        ...
        const result = await DB.execute(`SELECT user_id, token FROM ${tabela} WHERE user_email = '${data.email}' AND user_password = '${data.password}';`);
...
async function checkToken(token){
    const result = await DB.execute(`SELECT * FROM ${tabela} WHERE token = '${token}';`);
    return result;
} 
